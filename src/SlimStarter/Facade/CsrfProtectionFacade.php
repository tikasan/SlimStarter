...
class CsrfProtectionFacade extends \SlimFacades\Facade
{
    ...
    public static function getAsHiddenInput( $key )
    {
        // try to get token from SESSION before generating a new one
        $token = isset($_SESSION[ 'csrf_' . $key ]) ? $_SESSION[ 'csrf_' . $key ] : static::generate( $key );
